package org.example.Fuctions.Type;
...
@Slf4j
@NoArgsConstructor
public class ActeNaissanceLineRecordReader extends RecordReader<ActeDeNaissanceLongWritable, ActeDeNaissanceText> {
    ...
    private ActeDeNaissanceLongWritable key;
    private ActeDeNaissanceText value;
    private boolean isCompressedInput;
    private Decompressor decompressor;
    private byte[] recordDelimiterBytes;


    public ActeNaissanceLineRecordReader(byte[] recordDelimiter) {
    ...
    public void initialize(InputSplit genericSplit, TaskAttemptContext context) throws IOException {
        ...
        if (this.start != 0L) {
            this.start += (long)this.in.readLine(new ActeDeNaissanceText(), 0, this.maxBytesToConsume(this.start));
        }
    ...
    public boolean nextKeyValue() throws IOException {
        if (this.key == null) {
            this.key = new ActeDeNaissanceLongWritable();
        }

        this.key.set(this.pos);
        if (this.value == null) {
            this.value = new ActeDeNaissanceText();
        }
    ...
    public ActeDeNaissanceLongWritable getCurrentKey() {
        return this.key;
    }

    public ActeDeNaissanceText getCurrentValue() {
        return this.value;
    }
